...
export const classSchema = z.object({
  ...
  section: z.string().max(50, 'Section name too long').optional().nullable(),
});
...
export const gradeWeightsSchema = z.object({
  ...
  (data) => Math.abs(data.attendanceWeight + data.quizWeight + data.examWeight - 100) < 0.01,
...
export const studentSchema = z.object({
  ...
  parentName: z.string().max(100, 'Parent name too long').optional().nullable().or(z.literal('')),
  parentPhone: z.string().max(20, 'Parent phone too long').optional().nullable().or(z.literal('')),
...
export const scoresArraySchema = z.object({
